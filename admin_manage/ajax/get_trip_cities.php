...
if ($userRole !== 'admin') {
    respond(false, 'Only admin can access trip cities.');
}
...
function get_string($field, $default = '')
{
    ...
    return (string)($field['stringValue'] ?? $default);
}
...
try {
    ...
    $url = "https://firestore.googleapis.com/v1/projects/{$projectId}/databases/(default)/documents/trip_destinations?pageSize=200";
    [$status, $response] = firestore_request('GET', $url, $accessToken);

    if ($status < 200 || $status >= 300 || !$response) {
        respond(false, 'Failed to fetch trip cities from Firebase.');
    }

    $json = json_decode($response, true);
    if (!$json) {
        respond(false, 'Invalid Firebase response.');
    }
    ...
    foreach (($json['documents'] ?? []) as $doc) {
        ...
        $cities[] = [
            ...
            'description' => get_string($fields['description'] ?? null),
            'center' => [
    ...
    respond(true, 'Trip cities loaded successfully.', [
        'cities' => $cities
    ]);
